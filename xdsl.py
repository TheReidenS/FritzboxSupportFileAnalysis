...
def creationDateMapper(groups):
    ...
    cest_pattern = r'\w{3} \w{3} \d{2} \d{2}:\d{2}:\d{2} CEST \d{4}'
    cet_pattern = r'\w{3} \w{3} \d{2} \d{2}:\d{2}:\d{2} CET \d{4}'

    if re.match(cest_pattern, date_string):
        parsed_time = datetime.strptime(date_string, '%a %b %d %H:%M:%S CEST %Y')
    elif re.match(cet_pattern, date_string):
        parsed_time = datetime.strptime(date_string, '%a %b %d %H:%M:%S CET %Y')
    else:
        # Handle case if neither CEST nor CET is found
...
patterns = {
    ...
    'ipv4': r'External address status:\n\tIPv4:\s(\d+.\d+.\d+.\d+)',
    'ipv6': r'External address status:\n.*\n\tIPv6:\s(.*)',
